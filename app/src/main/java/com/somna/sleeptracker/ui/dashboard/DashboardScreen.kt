    ...
    Surface(color = DeepBlack, modifier = Modifier.fillMaxSize()) {
        LazyColumn(
            ...
            item {
                if (state.isLoading) {
                    LoadingCard()
                } else if (state.lastNight != null) {
                    LastNightCard(state.lastNight!!)
                } else {
                    EmptySleepCard()
                }
            }
            ...
            if (!state.isLoading && state.lastNight != null) {
                item {
                    InsightRow(state.lastNight!!)
                }
            }

            state.socialJetlagHours?.let { hours ->
                item {
                    SocialJetlagCard(hours)
                }
            }

            if (state.weeklyTrend.isNotEmpty()) {
                item {
                    SectionTitle("7-day sleep trend", "Your recent sleep duration")
                }
                item {
        ...
        Column(Modifier.padding(22.dp)) {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.Top) {
                Column {
                    ...
                    Text(
                        String.format("%.1f", night.durationHours),
                        color = SoftWhite,
                        fontSize = 48.sp,
                        fontWeight = FontWeight.Bold
                    )
            ...
            Box(
                ...
                Box(
                    Modifier
                        .fillMaxWidth(0.78f)
                        .fillMaxHeight()
                        .background(IndigoAccent, RoundedCornerShape(50))
                )
            }
        }
    }
...
private fun ConfidenceBadge(percent: Int, high: Boolean) {
    Surface(
        shape = RoundedCornerShape(12.dp),
        color = if (high) SuccessGreen.copy(alpha = 0.12f) else WarningAmber.copy(alpha = 0.12f)
    ) {
        Column(Modifier.padding(horizontal = 12.dp, vertical = 9.dp), horizontalAlignment = Alignment.CenterHorizontally) {
            Text("${percent}%", color = if (high) SuccessGreen else WarningAmber, fontSize = 18.sp, fontWeight = FontWeight.Bold)
...
private fun SocialJetlagCard(hours: Float) {
    ...
    Surface(shape = RoundedCornerShape(16.dp), color = CardSurface, modifier = Modifier.fillMaxWidth()) {
        Row(Modifier.padding(16.dp), verticalAlignment = Alignment.CenterVertically) {
            ...
            Text(
                if (severe) "Worth watching" else "Looks healthy",
                color = if (severe) WarningAmber else SuccessGreen,
                fontSize = 12.sp,
...
@Composable
private fun TrendCard(trend: List<SleepTrendUiModel>) {
            ...
            trend.forEach { day ->
                Row(verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Box(
                        ...
                        Box(
                            Modifier
                                .fillMaxWidth((day.durationHours / maxHours).coerceIn(0.04f, 1f))
                                .fillMaxHeight()
                                .background(IndigoAccent, RoundedCornerShape(50))
                        )
                    }
